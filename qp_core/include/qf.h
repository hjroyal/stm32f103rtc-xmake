...
*
* @details
* QPSet represents the set of Active Objects in the priority order.
* The set is capable of storing up to #QF_MAX_ACTIVE elements.
*/
typedef struct {
/* public: */
...
#ifndef Q_UNSAFE
/*! udate the Duplicate Inverse Storage of QPSet (QP FuSa Subsystem)
...
/*! trigger the QTicker active object to perform tick processing.
* @private @memberof QTicker
*/
void QTicker_tick_(
    QActive * const me,
...
* - except the firt event-pool 0, the event-size of the previously
...
* QF provides a native memory pool implementation (::QMPool). The macro
* #QF_EPOOL_TYPE_ determines the type of event pool used by a particular
* QF port.
*
* @note
...
* might choose not to use mutable events. In that case calling QF_poolInit()
...
*
* @details
* This function implements a simple garbage collector for the mutable events.
* Only mutable events are candidates for recycling. (A mutable event is one
* that is allocated from an event-pool, which is determined as non-zero
* QEVT_POOLID_(e)) Next, the function decrements the reference counter
...
* The mutable event is recycled by returning it to the pool from which
...
* mutable events to the "raw" thread-safe queues (see ::QEQueue). Such
...
/*${QF-macros::Q_NEW} ......................................................*/
#ifdef Q_EVT_CTOR
/*! Asserting allocate a mutable event
...
/*${QF-macros::Q_NEW_X} ....................................................*/
#ifdef Q_EVT_CTOR
/*! Non-asserting allocate a mutable event
* (case when ::QEvt is not a POD)
*/
...
* disabled, the QACTIVE_POST() macro does not pass the `sender_`
...
#ifdef Q_SPY
/*! Trigger the QTicker AO to perform tick processing.
*
* @details
* This macro is the recommended way to trigger clock tick processing,
* because it provides the vital information for software tracing and
* avoids any overhead when the tracing is disabled.
*
...
*            (macro #Q_SPY is defined)
* @note
* When QS software tracing is disabled, the macro calls QTicker_tick_()
* without the `sender` parameter, so the overhead of passing this
* extra parameter is entirely avoided.
*
* @note
* The pointer to the sender object is not necessarily a pointer
* to an active object. In fact, when QTICKER_TICK_X() is called from
* an interrupt, you would create a unique object just to unambiguously
* identify the ISR as the sender of the time events.
*
* @sa QTicker_tick_()
