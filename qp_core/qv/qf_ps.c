    ...
    enum_t const maxSignal)
{
    ...
    #ifndef Q_UNSAFE
        QPSet_update(&subscrSto[sig].set, &subscrSto[sig].set_dis);
    #endif
    }
}
    ...
    enum_t const sig)
{
    ...
    QS_BEGIN_NOCRIT_PRE_(QS_QF_ACTIVE_SUBSCRIBE, p)
    ...
    #ifndef Q_UNSAFE
    QPSet_update(&QActive_subscrList_[sig].set,
                 &QActive_subscrList_[sig].set_dis);
    #endif
    ...
    enum_t const sig)
{
    ...
    QS_BEGIN_NOCRIT_PRE_(QS_QF_ACTIVE_UNSUBSCRIBE, p)
        QS_TIME_PRE_();    /* timestamp */
        QS_SIG_PRE_(sig);  /* the signal of this event */
        QS_OBJ_PRE_(me);   /* this active object */
    QS_END_NOCRIT_PRE_()
