...
{
    uint_fast8_t const poolId = QF_maxPool_;

    /* see precondition{qf_dyn,200} and precondition{qf_dyn,201} */
    QF_CRIT_STAT_
    QF_CRIT_E_();
    Q_REQUIRE_NOCRIT_(200, poolId < QF_MAX_EPOOL);
    if (poolId > 0U) {
        Q_REQUIRE_NOCRIT_(201,
            QF_EPOOL_EVENT_SIZE_(QF_ePool_[poolId - 1U]) < evtSize);
    }
    QF_maxPool_ = poolId + 1U; /* one more pool */
    QF_CRIT_X_();

    /* perform the port-dependent initialization of the event-pool */
    QF_EPOOL_INIT_(QF_ePool_[poolId], poolSto, poolSize, evtSize);

    #ifdef Q_SPY
    /* generate the object-dictionary entry for the initialized pool */
    {
        uint8_t obj_name[9] = "EvtPool?";
        obj_name[7] = (uint8_t)(((uint8_t)'0' + poolId + 1U) & 0x7FU);
        QF_CRIT_E_();
        QS_obj_dict_pre_(&QF_ePool_[poolId], (char const *)obj_name);
        QF_CRIT_X_();
    }
    #endif /* Q_SPY*/
}
...
void QF_gc(QEvt const * const e) {
    ...
    if (poolNum != 0U) { /* is it a pool event (mutable)? */
        ...
        if (e->refCtr_ > 1U) { /* isn't this the last reference? */
            ...
            Q_ASSERT_NOCRIT_(410, (poolNum <= QF_maxPool_)
    ...
    #ifdef Q_SPY
            QF_EPOOL_PUT_(QF_ePool_[poolNum - 1U], (QEvt *)e,
                          (uint_fast8_t)QS_EP_ID + QEVT_POOLID_(e));
    #else
