    ...
    uint_fast16_t const blockSize)
{
    ...
    Q_REQUIRE_NOCRIT_(100, (poolSto != (void *)0)
            && (poolSize >= (uint_fast32_t)sizeof(QMBlock))
            && ((uint_fast16_t)(blockSize + sizeof(QMBlock)) > blockSize));

    me->free_head = (QMBlock *)poolSto;

    /* round up the blockSize to fit an integer # free blocks, NO DIVISION */
    me->blockSize = (QMPoolSize)sizeof(QMBlock);  /* start with just one */

    /* #free blocks that fit in one memory block */
    uint_fast16_t nblocks = 1U;
    while (me->blockSize < (QMPoolSize)blockSize) {
        me->blockSize += (QMPoolSize)sizeof(QMBlock);
        ++nblocks;
    }
    ...
    fb->next  = (QMBlock *)0;    /* the last link points to NULL */
    #ifndef Q_UNSAFE
    fb->next_dis = (uintptr_t)(~Q_UINTPTR_CAST_(fb->next));
    #endif
    ...
    uint_fast8_t const qs_id)
{
    ...
    if (me->nFree > (QMPoolCtr)margin) {
        ...
        Q_ASSERT_NOCRIT_(300, fb != (QMBlock *)0);
        ...
        if (me->nFree == 0U) {
            ...
            Q_ASSERT_NOCRIT_(320, fb_next == (QMBlock *)0);
